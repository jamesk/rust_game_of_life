...
fn do_life(sectionTop: Arc<Mutex<BoardSection>>,
           sectionBottom: Arc<Mutex<BoardSection>>,
           iteration: usize) {
...
fn main() {
    ...
    let total_rows = sectionTop.lock().unwrap().get_board().get_height() +
                     sectionBottom.lock().unwrap().get_board().get_height() -
                     1;
